...
const mensagens = require('@utils/mensagens') // Certifique-se de importar mensagens aqui se usar
...
async function handlePedirCommand(sock, msg, args) { // <--- Ordem Corrigida!
  ...
  if (estadosDeConversa.has(userId)) {
    ...
    await sock.sendMessage(userId, { text: mensagens.gerais.saudacaoPedido || 'Opa! O que você gostaria de pedir?' }) // Use sua mensagem de saudação
  }
